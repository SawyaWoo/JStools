...
// 7.从localStorage里面根据指定的键(key)获取一个数组 
...
// 8.将一个数组(arr)以指定的键(key)存储到localStorage里面 
kits.saveLocalDataArray = function (key, arr) {
  var data = JSON.stringify(arr);
  localStorage.setItem(key, data);
};

// 9.根据对应的id从localStorage中指定键(key)的数组中删除一条数据
...
// 10.根据id修改localStorage里面的指定键(key)的数组数据  
